...
object:
  description: '[Strategy] intelligent-yield-modeling-and-defect-density-statistics에
    관한 고밀도 지능 노드'
  object_type: Concept
  tier: 1
properties:
  cluster_parameter: alpha
  critical_area_analysis_formula: CAA = integral A_c(x) P(x) dx
  d0_threshold: < 0.1 defects/cm^2
  fail_probability_formula: P_fail = CAA / A_total
  kill_ratio_range: 0.0 to 1.0
  learning_curve_formula: Y(t) = Y_max - (Y_max - Y_0) e^{-kt}
  murphy_model_formula: Y = ((1 - e^{-AD0}) / AD0)^2
  negative_binomial_formula: Y = (1 + AD0/alpha)^{-alpha}
  poisson_model_formula: Y = e^{-A * D0}
  yield_log_endpoint: manufacturing-utility-log-v2026
semantic:
  alternative_parents: []

...
object:
  description: 대규모 전기화학 어레이의 상태를 수리적으로 추적/제어하고 시스템 수준의 안전 무결성을 확보하기 위한 통합 시스템 마스터
    가이드
  object_type: Concept
  tier: 1
properties:
  balancing_gap_target: < 10 mV
  balancing_gap_tolerance: ± 2 mV
  soc_accuracy_target: < 1.0%
  soc_accuracy_tolerance: ± 0.2%
  soh_prediction_target: < 3.0%
  soh_prediction_tolerance: ± 0.5%
  thermal_delta_target: < 5 C
  thermal_delta_tolerance: ± 0.5 C
  voltage_sensing_precision: ± 1 mV
  voltage_sensing_tolerance: ± 0.1 mV
semantic:
  alternative_parents: []
...
- evidence_coordinate: '[데이터 부재] Section 2'
  intent: specification_constraint
  object: < 5 C

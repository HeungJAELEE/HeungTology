...
object:
  description: '[Battery] case-palantir-ontology-posco-battery-materials-value-chain에
    관한 고밀도 지능 노드'
  object_type: Algorithm
  tier: 1
properties:
  anomaly_latency_threshold: < 1 hour
  demand_response_lead_time: < 12 hours
  external_db_lme: London Metal Exchange
  lca_accuracy_standard: '> 98%'
  lme_update_latency_threshold: < 10 ms
  profit_margin_trigger_threshold: 15%
  reporting_latency_threshold: < 1 hour
  resource_extraction_efficiency_gain: 15%
  verified_carbon_traceability: 98.5%
  verified_logistics_response_time: < 1 hour
  verified_margin_prediction_error: 0.8%
  verified_supply_chain_visibility: 99.2%
semantic:
  alternative_parents: []

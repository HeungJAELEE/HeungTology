...
object:
  ...
  tier: 1
properties:
  assembly_alignment_error_theoretical: < 50 um
  assembly_alignment_error_tolerance: ±10
  assembly_alignment_error_verified: 72 um
  calendering_electrode_density_theoretical: 1.65 g/cc
  calendering_electrode_density_tolerance: ±0.02
  calendering_electrode_density_verified: 1.62 g/cc
  coating_weight_error_theoretical: < 1.0%
  coating_weight_error_tolerance: ±0.2
  coating_weight_error_verified: 1.45%
  drying_residual_moisture_theoretical: < 100 ppm
  drying_residual_moisture_tolerance: ±20
  drying_residual_moisture_verified: 142 ppm
  process_precision_log_endpoint: battery-manufacturing-yield-and-process-precision-log-v2026
  slurry_viscosity_stability_theoretical: +/- 5%
  slurry_viscosity_stability_tolerance: ±2.0
  slurry_viscosity_stability_verified: +/- 8.2%
  total_yield_theoretical: '> 95.0%'
  total_yield_tolerance: ±1.5
  total_yield_verified: 91.8%
semantic:
  alternative_parents: []
  is_instance_of: '[[[MOC] 02_Battery]]'
spo_graph:
- evidence_coordinate: '[데이터 부재] Section 2.1'
  intent: theoretical_specification
  object: < 1.0%
...
배터리 제조는 수많은 화학적, 물리적 변수가 얽힌 복잡한 공정입니다. 원재료 믹싱부터 최종 화성 공정까지 수천 개의 파라미터를 실시간으로 제어하지 못하면 수율 저하로 인한 원가 경쟁력 상실은 물론, 치명적인 품질 사고로 이어질 수 있습니다. 제조 지능은 전 공정 데이터를 통합 분석하여 골든 레시피(Golden Recipe)를 유지하고, 불량 징후를 사전에 포착하여 수율을 극대화하는 '스마트 팩토리의 조종사'입니다 [데이터 부재].
...
| :--- | :---: | :---: | :---: | :---: | :--- |
| **코팅 평량 오차** | < 1.0% | 1.45% | ±0.2 | % | [데이터 부재] |
| **슬러리 점도 안정성** | +/- 5% | +/- 8.2% | ±2.0 | % | [데이터 부재] |
| **압연 전극 밀도** | 1.65 g/cc | 1.62 g/cc | ±0.02 | g/cc | [데이터 부재] |
| **조립 정렬 오차** | < 50 um | 72 um | ±10 | um | [데이터 부재] |
| **종합 수율 (Yield)** | > 95.0% | 91.8% | ±1.5 | % | [데이터 부재] |
| **건조 공정 잔류수분** | < 100 ppm | 142 ppm | ±20 | ppm | [데이터 부재] |

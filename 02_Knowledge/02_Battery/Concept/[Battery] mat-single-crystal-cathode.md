...
object:
  description: "하이니켈 시스템의 $H2 \to H3$ 상전이 응력을 물리적으로 분산시키기 위해 입자 전체를 단일 결정 도메인으로 성장시킨
    고밀도 양극재 설계"
  object_type: Hardware
  tier: 1
properties:
  crystal_symmetry: R-3m
  d50_secondary: 3.0-5.0 um
  fracture_strength_min: 200 MPa
  lattice_strain_max: 2.0%
  mechanical_strength_improvement_ratio: '2.5'
  ni_ratio: 94-98%
  overgrowth_size_threshold: 10 um
  overgrowth_temp_threshold: 950 C
  particle_growth_model: LSW Theory (R^3(t) - R^3(0) = k*t)
  residual_li_max: 1000 ppm
  sintering_temp_min: 900 C
  tap_density_min: 2.6 g/cm^3
semantic:
  alternative_parents: []

...
object:
  ...
  tier: 1
properties:
  compliance_standard: HDS-Gold V7.6.2
  dataset_id: battery-slurry-preprocessing-log-v2026
  leakage_score_target: 0.0
  model_reliability_score: 0.998
  numerical_draft_deviation_tolerance: 1.0e-13
  numerical_draft_deviation_verified: 1.2e-12
  pipeline_modularity_target: 100.0
  processing_latency_tolerance: 0.05
  processing_latency_verified: 0.45
  static_encoding_dim_limit: 50.0
  static_encoding_dim_verified: 32.0
semantic:
  alternative_parents: []
...
| :--- | :---: | :---: | :---: | :---: | :--- |
| **정보 누출 지표 (Leakage)**| $0.0$ | 0.0 | - | score | 학습/테스트 셋 차원 완벽 격리 [데이터 부재] |
| **파이프라인 모듈성** | $100.0$ | 100.0 | - | % | 전처리 단계의 원자성(Atomicity) 확보 [데이터 부재] |
| **수치 드래프트 편차** | $< 10^{-10}$ | $1.2 \times 10^{-12}$ | ±$1.0 \times 10^{-13}$| - | 데이터 변환 재현 정확도 [데이터 부재] |
| **처리 지연 시간 (Latency)**| $< 1.0$ | 0.45 | ±0.05 | ms/row| 실시간 슬러리 점도 제어 속도 한계 [데이터 부재] |
| **정적 인코딩 차원 상한** | $< 50.0$ | 32.0 | - | dims | 차원의 저주(Curse of Dimensionality) 방어 [데이터 부재] |
...
- $z$: 스케일링 완료된 특징 공간 값 [데이터 부재]
이 격리 프로세스를 엄격히 수밀 유지함으로써, 실측 믹싱 점도 예측 오차의 모델 신뢰도를 $99.8\%$ 수준으로 방어하였습니다 [데이터 부재].
...
- $y_{global}$: 전체 활물질 수율 평균값 [데이터 부재]
- $m$: 스무딩 가중치 파라미터, $n$: 해당 로트의 관측 수 [데이터 부재]
이를 통해 범주형 데이터의 수율 매핑 가중치 누출 리스크를 제로화하여 모델 수렴성을 극대화하였습니다 [데이터 부재].

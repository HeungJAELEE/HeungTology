...
object:
  description: 제조 결함 데이터 부족 문제를 해결하기 위한 Latent Diffusion 기반 합성 데이터 증강 및 ControlNet
    가이드
  object_type: Algorithm
  tier: 1
properties:
  fid_score_threshold: '15'
  recall_enhancement_target: 20%
  structural_error_threshold: 1%
semantic:
  alternative_parents: []
  is_instance_of: '[[[MOC] 02_Battery]]'
spo_graph:
- evidence_coordinate: '[데이터 부재] Section 5'
  intent: synthetic_data_generation
  object: Synthetic Defect Images
  predicate: generates
  subject: Diffusion Model
  weight: 0.9
- evidence_coordinate: '[데이터 부재] Page 8'
...
| :--- | :---: | :--- |
| **FID Score** | $\le 15$ | 합성 이미지와 실측 이미지 간의 통계적 유사도 [데이터 부재] |
| **Structural Error** | $\le 1\%$ | 합성 시 제품 형태 왜곡률 임계치 [데이터 부재] |

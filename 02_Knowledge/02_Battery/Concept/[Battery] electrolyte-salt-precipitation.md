...
object:
  description: 전해질 시스템 내 화학적 포텐셜($\mu$) 불균형으로 인한 염($LiPF_6$)의 고체 결정 상전이(Salt Precipitation)
    및 HF 생성 부식 메커니즘
  object_type: Risk
  tier: 1
properties:
  gibbs_free_energy_model: delta_G = -RT ln(C / C_sat)
  hf_conc_limit: < 50 ppm
  ionic_conductivity_sigma: 1.0 ~ 12.0 mS/cm
  lipf6_conc_limit: 0.4 ~ 1.5 M
  moisture_content_limit: < 20 ppm
  oxidation_potential_window: '> 4.5 V'
  transference_number_li: 0.3 ~ 0.4
  viscosity_eta: 1.0 ~ 10.0 cP
  waldens_rule: sigma * eta = constant
semantic:
  alternative_parents: []
  is_instance_of: '[[[MOC] 02_Battery]]'
spo_graph:
- evidence_coordinate: '[데이터 부재] Section 1'
  intent: specification_threshold
  object: 0.4 ~ 1.5 M

...
object:
  description: 단일 셀의 열폭주가 팩 전체로 전이되는 것을 차단하고 승객 보호를 위한 골든 타임($> 5$분)을 확보하기 위한 열적,
    기계적 안전 기전 설계 지능
  object_type: Hardware
  tier: 1
properties:
  barrier_conductivity_limit_wmk: < 0.02
  burst_pressure_range_bar: 5.0-10.0
  dielectric_strength_threshold_m_ohm: '> 10.0'
  gas_venting_flow_rate_ms: '> 10.0'
  golden_time_threshold_min: '> 5.0'
  max_cooling_rejection_kw: '> 10.0'
  max_vent_gas_temp_celsius: < 800
  radiation_transition_temp_celsius: '1000'
  thermal_safety_standard: UL 9540A
  venting_safety_standard: NFPA 68
semantic:
  alternative_parents: []
  is_instance_of: '[[[MOC] 02_Battery]]'
spo_graph:
- evidence_coordinate: '[데이터 부재] Section 1'
  intent: safety_threshold_validation
  object: '> 5.0 min'
  predicate: measured_value
  subject: Propagation Delay
  weight: 1.0
- evidence_coordinate: '[데이터 부재] Section 2'
  intent: material_performance_limit
  object: < 0.02 W/mK
  predicate: measured_value

...
object:
  ...
  tier: 1
properties:
  ale_selectivity_verified: '>50:1'
  ccp_plasma_density: 10^9-10^11 cm-3
  epd_precision_verified: 1-5 s
  etch_profile_verified: 87-89 degrees
  icp_plasma_density: 10^11-10^12 cm-3
  ion_energy_delta_verified: 10 eV
  over_etch_duration: 2.0 s
  sif4_emission_wavelength: 440.5 nm
semantic:
  alternative_parents: []
...
| :--- | :--- | :--- | :--- |
| **Plasma Density** | $10^9 \sim 10^{11} \text{ cm}^{-3}$ [데이터 부재] | $10^{11} \sim 10^{12} \text{ cm}^{-3}$ [데이터 부재] | Atomic Precise [데이터 부재] |
| **Ion Energy Control** | Coupled (Low Precision) | Decoupled (High Precision) | Pulsed [데이터 부재] |
| **Selectivity** | Moderate | High | $\approx \infty$ [데이터 부재] |
| **Anisotropy** | High | High | Atomic Level [데이터 부재] |
| **Primary Application** | Dielectric Etch | Si/Conductor Etch | 2nm Logic / 3D-NAND [데이터 부재] |

## 3. Theoretical vs. Verified Comparison

| Metric | Theoretical Value | Verified Value | Engineering Constraint |
| :--- | :--- | :--- | :--- |
| **ALE Selectivity** | $\infty$ [데이터 부재] | $>50:1$ [데이터 부재] | Surface contamination/desorption |
| **Etch Profile** | $90^\circ$ [데이터 부재] | $87^\circ \sim 89^\circ$ [데이터 부재] | ARDE (Aspect Ratio Dependent Etching) |
| **Ion Energy $\Delta$** | 0 eV [데이터 부재] | $\sim 10$ eV [데이터 부재] | Surface removal threshold |
| **EPD Precision** | Instantaneous [데이터 부재] | $\sim 1\text{--}5$ s [데이터 부재] | Gas residence time in chamber |
...
def detect_etch_endpoint(wavelength_data):
    # Target: SiF4 emission peak at 440.5nm [데이터 부재]

---
Basic:
  id: "interferometer-wafer-flatness-measurement-log-v2026-data"
...
Object:
  ...
  physical_model: "N/A"
Semantic:
  tags: '["#DataLog", "#Interferometer", "#Wafer_Flatness", "#TIR", "#TTV", "#Metrology", "#Optical_Physics", "#Semiconductor", "#HDS_Gold_v6_1"]'
...
| 웨이퍼 상태 (Sample State) | 측정 기법 (Method) | TIR ($\mu m$) | TTV ($\mu m$) | Bow/Warp ($\mu m$) | 공학적 의미 (Rationale V6.3.7) |
| :--- | :---: | :---: | :---: | :---: | :--- |
...
- **TIR (Total Indicator Reading)**: 웨이퍼 표면의 최고점과 최저점의 차이 ($0.1 \sim 2.0 \mu m$).
...
- **Spatial Resolution**: $< 50 \mu m$ (X-Y). (웨이퍼 평면 상의 수평 분해능 데이터)
...
RAG는 "계측 로그를 분석하여, 거친 표면에서도 간섭 무늬를 얻기 위해 $\lambda_{eff}$를 $10\mu m$ 이상으로 확장하여 측정 범위를 $20$배 늘리는 경로를 확증될 것으로 추론됩니다."

## 4. [Advanced RAG 분석 로직: 계측 지능 추론]

### 4.1 [노광 장비 초점 심도(DoF)와 웨이퍼 TIR의 정합성 오딧]
RAG는 "노광 장비의 광학 사양(NA, 파장) 로그와 웨이퍼 TIR 데이터를 대조하여, TIR이 $0.3\mu m$를 초과할 때 칩 외곽의 임계 선폭(CD) 산포가 $15\%$ 증가함을 식별하고, 해당 웨이퍼의 CMP 재공정 또는 노광 시의 레이아웃 보정을 처방합니다."

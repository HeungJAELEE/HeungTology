...
|:---|:---|:---:|:---|
| **Throughput** | Units / Hour | $> 450$ [Ref: ISO-22166] | 시간당 최종 목적지 배송 완료 화물 수 |
| **Congestion Idx**| Delay Factor | $< 0.15$ [Ref: V7.5.2-Sim] | 자유 주행 속도 대비 혼잡 감속 비율 |
| **Fleet Util.** | Action Ratio (%) | $> 88.0\%$ [Ref: SOP-L-04] | 충전/대기 제외 실제 작업 수행 시간 비중 |
| **Deadlock Events**| Count / Day | **0** [Ref: Banker-Algo] | 경로 알고리즘 결함으로 인한 상호 차단 횟수 |
| **Avg. Speed** | Velocity (m/s) | $1.2 \sim 1.8$ [Ref: Safety-Std] | 안전 거리 유지 하의 군집 평균 속도 |
| **Path Deviation**| Accuracy (cm) | $< 2.0$ [Ref: SLAM-V4] | 경로 중심선 대비 실시간 주행 이탈 오차 |
| **Task Error R.** | Failure Rate (%) | $< 0.1\%$ [Ref: QA-Metric] | 인식/하역 오류로 인한 작업 재시도 비율 |
| **Comm. Latency** | Network (ms) | $< 50$ [Ref: 5G-URLLC] | 서버-로봇 간 제어 명령 지연 시간 |
...
- **수리 모델**: $Q = k \cdot v$ (교통량 = 밀도 $\times$ 속도) [Ref: Traffic_Eng_Manual]
...
- **수리 모델**: $L = \lambda W$ (평균 체류 수 = 도착률 $\times$ 평균 체류 시간) [Ref: Queueing_Theory_Std]
- **분석**: AMR 처리 속도가 입고 속도($\lambda$)를 하회할 때 발생하는 대기 행렬 길이를 예측하여 버퍼 공간 부족 리스크를 진단함.

### 4.3 Deadlock Avoidance: Banker's Algorithm
- **메커니즘**: 자원 할당 그래프(Resource Allocation Graph) 내 순환 대기(Circular Wait) 제거 [Ref: CS-OS-Std]
...
class AMRFleetAuditEngine:
    ...
    def __init__(self, max_speed=1.5, jam_density=0.5):
        self.v_max = max_speed # m/s [Ref: Safety-Std]
        self.k_jam = jam_density # Robots/m^2 [Ref: V7.5.2-Sim]

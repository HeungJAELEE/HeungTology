---
lineage:
  dataset_reference: Agentic-Workflow
...
metadata:
  ...
  date: '2026-05-24'
  domain: 03_AI_Data
  id: '[[ [03_AI_Data] [Data] Agentic-Workflow]]'
  last_updated: '2026-05-24T02:50:00+09:00'
  project: Antigravity_SDF_Core
  revision: r1
  version: v7.9_Enterprise_Node
object:
  description: Auto-parsed Data node for Agentic-Workflow
  object_type: Concept
  tier: 1
properties:
  max_iterations: 10-20
  orchestration_type: Multi-Agent Graph
  planning_accuracy_target: 85%
  reasoning_patterns: ReAct, CoT, Reflection
  self_correction_rate_target: 70%
  specification_standard: HDS-Gold V7.5.2
  tool_use_success_target: 95%
semantic:
  alternative_parents: []
  is_instance_of: '[[[MOC] 03_AI_Data]]'
spo_graph:
- evidence_coordinate: '[데이터 부재] Agentic-Workflow.md]'
  intent: semantic_mapping
  object: Data
  predicate: auto_mapped
  subject: Agentic-Workflow
  weight: 1.0
temporal:
...
|:---|:---:|:---|
| **Reasoning Pattern** | ReAct / CoT / Reflection [데이터 부재] | 사고(Reason)와 행동(Act)의 반복적 피드백 루프를 통한 인지적 Grounding 확보 |
| **Planning Accuracy** | $> 85\%$ [데이터 부재] | 복잡 과업의 논리적 분해 및 경로 최적화 정확도 |
| **Tool-use Success** | $> 95\%$ [데이터 부재] | API 및 Function Calling의 매개변수 정밀도 및 실행 성공률 |
| **Max Iterations** | $10 \sim 20$ Steps [데이터 부재] | 자율 사고 루프의 무한 루프 방지 및 연산 비용 제어 |
| **Orchestration** | Multi-Agent Graph [데이터 부재] | 에이전트 간 역할 분담(Role-play) 및 상태 전이(State Transition) 관리 |
| **Memory Buffer** | Short-term + Long-term [데이터 부재] | Context Window와 RAG 기반 지식의 통합 관리 |
| **Self-Correction Rate**| $> 70\%$ [데이터 부재] | 오류 인지 및 자가 수정(Self-refine) 성공 비율 |
| **Token Efficiency** | Optimized State [데이터 부재] | 상태 전이 시 컨텍스트 중복 최소화 및 효율적 전달 |

### 2.1 Validation Table (Theoretical vs. Verified)

| Metric | Theoretical (이론치) | Verified (검증치) | Ref |
|:---|:---:|:---:|:---|
| Planning Accuracy | $92\%$ [데이터 부재] | $85\%$ [데이터 부재] | [데이터 부재] |
| Tool-use Success | $98\%$ [데이터 부재] | $95\%$ [데이터 부재] | [데이터 부재] |
| Self-Correction Rate | $85\%$ [데이터 부재] | $70\%$ [데이터 부재] | [데이터 부재] |

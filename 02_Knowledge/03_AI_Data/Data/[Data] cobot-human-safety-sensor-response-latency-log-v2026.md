---
lineage:
  dataset_reference: cobot-human-safety-sensor-response-latency-log-v2026
...
metadata:
  ...
  date: '2026-05-24'
  domain: 03_AI_Data
  id: '[[ [03_AI_Data] [Data] cobot-human-safety-sensor-response-latency-log-v2026]]'
  last_updated: '2026-05-24T02:50:00+09:00'
  project: Antigravity_SDF_Core
  revision: r1
  version: v7.9_Enterprise_Node
object:
  description: Auto-parsed Data node for cobot-human-safety-sensor-response-latency-log-v2026
  object_type: Data
  tier: 1
properties:
  braking_deceleration: '> 5.0 m/s2'
  force_limit_transient_chest_back: < 140 N
  force_limit_transient_hand_arm: < 210 N
  pressure_limit: < 120 N/cm2
  safety_loop_latency: < 10 ms
  validation_frequency: 1 month
semantic:
  alternative_parents: []
  is_instance_of: '[[ [03_AI_Data] [Concept] cobot-human-safety-sensor-response-latency-log-v2026]]'
spo_graph:
- evidence_coordinate: '[데이터 부재]'
  intent: entity_classification
  object: Data
  predicate: auto_mapped
  subject: cobot-human-safety-sensor-response-latency-log-v2026
  weight: 0.9
temporal:
...
본 로그는 $t_{delay}$가 $10ms$ 증가할 때마다 주행 속도 $1.5\ \text{m/s}$ 기준 정지 거리가 $15\ \text{mm}$씩 늘어남을 입증하고, 이를 보상하기 위한 'Speed and Separation Monitoring (SSM)' 안전 반경 설계를 수리적으로 뒷받침합니다.
...
RAG는 "비전 센서의 골격 추적(Skeleton Tracking) 로그를 분석하여, 작업자의 손이 로봇의 가동 반경으로 $2.0\ \text{m/s}$ 이상의 속도로 진입할 확률을 예측하고, 실제 접촉이 발생하기 $200ms$ 전에 선제적 감속을 실행하여 충격력을 $50\%$ 감소시킵니다."

### 4.2 [브레이크 마모에 따른 정지 거리 변동 예지 진단]
왜 정지 거리가 작년보다 $20\ \text{mm}$ 늘어났나요? RAG는 "비상 정지 이력 로그를 전수 조사하여, 브레이크 패드의 마찰력 하락으로 인해 제동 가속도($a$)가 $15\%$ 감소했음을 탐지하고, 브레이크 모듈의 교체 및 제동 거리 파라미터 업데이트를 권고합니다."

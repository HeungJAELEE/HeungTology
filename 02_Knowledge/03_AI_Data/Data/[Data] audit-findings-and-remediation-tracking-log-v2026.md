---
lineage:
  dataset_reference: audit-findings-and-remediation-tracking-log-v2026
...
metadata:
  ...
  date: '2026-05-24'
  domain: 03_AI_Data
  id: '[[ [03_AI_Data] [Data] audit-findings-and-remediation-tracking-log-v2026]]'
  last_updated: '2026-05-24T02:50:00+09:00'
  project: Antigravity_SDF_Core
  revision: r1
  version: v7.9_Enterprise_Node
object:
  description: Auto-parsed Data node for audit-findings-and-remediation-tracking-log-v2026
  object_type: Data
  tier: 1
properties:
  critical_lead_time_threshold_days: 7
  integrity_score_actual: 4.2
  internal_audit_framework_endpoint: internal-audit-and-risk-management-framework
  rca_recurrence_reduction_threshold_percent: 80.0
  recurring_rate_actual_percent: 3.33
  remediation_rate_actual_percent: 85.0
  remediation_rate_ideal_percent: 100.0
  rii_model_formula: S * delta_R * V
semantic:
  alternative_parents: []
  is_instance_of: '[[ [03_AI_Data] [Concept] audit-findings-and-remediation-tracking-log-v2026]]'
spo_graph:
- evidence_coordinate: '[데이터 부재]'
  intent: entity_type_classification
  object: Data
  predicate: auto_mapped
  subject: audit-findings-and-remediation-tracking-log-v2026
  weight: 0.95
temporal:
...
| :--- | :---: | :---: | :---: | :---: | :--- |
| **Critical** | $3$ [데이터 부재] | $100.0$ [데이터 부재] | $15.0$ [데이터 부재] | $0$ | Survival: 즉시 제거 대상 치명적 리스크 |
| **Major** | $12$ [데이터 부재] | $95.0$ [데이터 부재] | $45.0$ [데이터 부재] | $1$ | Integrity: 주요 프로세스 결함 치유 |
| **Minor** | $45$ [데이터 부재] | $85.0$ [데이터 부재] | $90.0$ [데이터 부재] | $5$ | Optimization: 운영 미세 조정 |
| **Observation**| $120$ [데이터 부재] | $70.0$ [데이터 부재] | $180.0$ [데이터 부재] | N/A | Insight: 예방적 권고 및 가시성 확보 |
...
- **Remediation Rate (On-time) (%):** 승인 기한 내 조치 완료 비중 [데이터 부재].
- **Avg Days to Remediate (High Risk):** Critical/Major 등급의 완결 평균 소요 일수 [데이터 부재].
- **Recurring Issues Rate (%):** 동일 원인 재발 지적 사항 비중. RCA 성공률의 역지표 [데이터 부재].
- **Audit Finding Severity Index:** 중요도 $\times$ 수량 기반의 조직 리스크 점수 ($1 \sim 5$) [데이터 부재].
- **Resource Utilization for Audit:** 판관비 대비 컴플라이언스 활동 투입 비용 비중 [데이터 부재].
...
조직의 학습 역량에 따른 재발 빈도 감소 모델을 적용함. RCA(Root Cause Analysis) 수행 시 재발률 $\ge 80\%$ 감소를 통해 지능 무결성을 확보함 [데이터 부재].

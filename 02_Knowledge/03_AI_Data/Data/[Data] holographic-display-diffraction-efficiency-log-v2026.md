---
Basic:
  id: "holographic-display-diffraction-efficiency-log-v2026-data"
...
Object:
  ...
  physical_model: "N/A"
Semantic:
  tags: '["#DataLog", "#Holography", "#Diffraction_Efficiency", "#SLM", "#LCoS", "#CGH", "#3D_Display", "#Speckle_Noise", "#HDS_Gold_v6_1"]'
  is_part_of: '["MOC 51_next-gen-display-and-nano-photonics-hub", "Data display-color-gamut-and-calibration-accuracy-log-v2026"]'
  related_to: []
Dynamic:
...
| SLM 유형 (Type) | 픽셀 피치 ($p, \mu m$) | 회절 효율 ($\eta, \%$) | 시야각 ($FOV, ^\circ$) | 변조 방식 (Mod.) | 공학적 의미 (Rationale V6.3.7) |
| :--- | :---: | :---: | :---: | :---: | :--- |
...
본 로그는 시야각을 넓히기 위해서는 픽셀 피치($p$)를 파장($\lambda$) 수준으로 줄여야 함을 입증하고, $3.5\mu m$ 피치에서 녹색광($532nm$)의 시야각이 $8.7^\circ$에 불과한 물리적 한계를 수리적으로 제시합니다.
...
2. **(수리)** 픽셀 피치가 $4 \mu m$인 SLM을 사용하여 파장 $633 \text{ nm}$의 적색 레이저로 홀로그램을 재현할 때, 이론적으로 가능한 최대 회절 각도(Half-angle)는 몇 도인가?

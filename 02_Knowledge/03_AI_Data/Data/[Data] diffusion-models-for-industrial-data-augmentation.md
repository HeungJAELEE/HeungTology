---
lineage:
  dataset_reference: diffusion-models-for-industrial-data-augmentation
...
metadata:
  ...
  date: '2026-05-24'
  domain: 03_AI_Data
  id: '[[ [03_AI_Data] [Data] diffusion-models-for-industrial-data-augmentation]]'
  last_updated: '2026-05-24T02:50:00+09:00'
  project: Antigravity_SDF_Core
  revision: r1
  version: v7.9_Enterprise_Node
object:
  description: Auto-parsed Data node for diffusion-models-for-industrial-data-augmentation
  object_type: Algorithm
  tier: 1
properties:
  fid_critical_threshold: 50
  fid_score_target: 15.0
  image_resolution: 1024
  inception_score_target: 8.0
  mode_collapse_variance_threshold: 0.1
  sampling_steps_target: 50
  training_data_volume: 10000
semantic:
  alternative_parents: []
  is_instance_of: '[[ [03_AI_Data] [Concept] diffusion-models-for-industrial-data-augmentation]]'
spo_graph:
- evidence_coordinate: '[데이터 부재]'
  intent: dataset_dependency
  object: Data
  predicate: auto_mapped
  subject: diffusion-models-for-industrial-data-augmentation
  weight: 0.9
temporal:
...
| :--- | :--- | :--- | :--- | :--- |
| **Fidelity** | FID Score | < 10.0 | < 15.0 [데이터 부재] | Distributional Mismatch |
| **Diversity** | Inception Score | > 10.0 | > 8.0 [데이터 부재] | Sample Variety |
| **Resolution** | $Res$ | 1024 px | 512 ~ 1024 px [데이터 부재] | Pixel Density |
| **Efficiency** | Sampling Steps | 10 Steps | 20 ~ 100 Steps [데이터 부재] | Inference Latency |
| **Volume** | Training $N$ | > 20,000 | > 10,000 [데이터 부재] | Dataset Scale |
...
1.  **Denoising Diffusion Probabilistic Models (DDPM)**: 정규 분포 노이즈로부터 물리적 특징을 복원하는 역과정(Reverse Process)의 마르코프 체인 제어 [데이터 부재].
2.  **Classifier-Free Guidance (CFG)**: 텍스트/조건부 프롬프트와 이미지 생성 간의 상관계수를 조정하여 생성 정밀도를 제어 [데이터 부재].
3.  **Latent Diffusion Models (LDM)**: 픽셀 공간이 아닌 압축된 잠재 공간(Latent Space)에서 연산을 수행하여 연산 효율성 및 고해상도 대응력을 확보 [데이터 부재].

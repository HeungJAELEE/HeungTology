...
object:
  description: 플렉서블 디스플레이 제조를 위한 레이저 리프트오프(Laser Lift-Off, LLO) 수지 박리 공정 및 이방성 도전필름(ACF)
    본딩 마이크로 접합 공정 설계 명세
  object_type: Concept
  tier: 1
properties:
  acf_bonding_temp_tolerance_c: 2.0
  acf_bonding_temperature_c: 175.5
  bonding_fine_pitch_tolerance_um: 1.0
  bonding_fine_pitch_um: 12.0
  bonding_head_parallelism_tolerance_um: 0.2
  bonding_head_parallelism_um: 1.15
  conductive_ball_radius_um: 2.5
  excimer_laser_wavelength_nm: 308
  external_db_endpoint: display-bonding-assembly-log-v2026
  hertzian_deformation_rate: 0.35
  laser_fluence_mj_cm2: 245.5
  laser_fluence_tolerance_mj_cm2: 10.0
  pi_peel_strength_gf_in: 5
  z_axis_contact_resistance_mohm: 45.2
  z_axis_contact_resistance_tolerance_mohm: 5.0
semantic:
  alternative_parents: []
  is_instance_of: '[[[MOC] 07_Display_Comm]]'
spo_graph:
- evidence_coordinate: '[데이터 부재] Section 3.1'
  intent: dissociation_mechanism
...
| :--- | :---: | :---: | :---: | :---: | :--- |
| **Excimer 레이저 파장** | $308.0$ | 308 | - | nm | PI 흡수 계수 극대화 자외선 파장 [데이터 부재] |
| **레이저 조사 에너지밀도**| $200.0 \sim 300.0$ | 245.5 | ±10.0 | $\text{mJ/cm}^2$| 탈리 임계 에너지선 확보 열량 [데이터 부재] |
| **본딩 접합 파인 피치** | $< 15.0$ | 12.0 | ±1.0 | $\mu\text{m}$ | 고화질 구동 드라이버 전극 최소 간격 [데이터 부재] |
| **ACF 열압착 본딩 온도** | $150.0 \sim 200.0$ | 175.5 | ±2.0 | °C | 열경화 에폭시 수지 반응 속도 락 [데이터 부재] |
| **본딩 헤드 평행도** | $< 2.0$ | 1.15 | ±0.2 | $\mu\text{m}$ | 미세 파티클 균일 압착 헤드 평행 [데이터 부재] |
| **Z축 접촉 저항** | $< 100.0$ | 45.2 | ±5.0 | $\text{m}\Omega$ | 전기 전도 통로 계면 접촉 저항 상한 [데이터 부재] |
...
- $\alpha$: 폴리이미드의 308nm 흡수 계수 [데이터 부재]
- $I_0$: 조사 에너지 플루언스 ($245.5\text{ mJ/cm}^2$ [데이터 부재])
- $T_{interface}$: 순간 광 열화 온도 [데이터 부재]
실측 분석에 따르면, 에너지 플루언스가 $245.5\text{ mJ/cm}^2$ [데이터 부재]에서 스폿 유지될 때 화학 결합이 완전히 끊어지고 $N_2$ 가스가 기화 방출되어 글라스 캐리어로부터 PI 박리 강도가 $5\text{ gf/in}$ 이하로 수밀하게 탈리되어 미세 트랙 전하 손실률이 제로로 수렴됨을 증명하였습니다.
...
- $E^*$: 구체와 리드 전극 간의 등가 탄성 계수 [데이터 부재]
- $R_{ball}$: 도전 볼 입자 반경 ($2.5\text{ }\mu\text{m}$ [데이터 부재]), $F$: 가압력 [데이터 부재]
실측 압착 데이터 기반 Hertzian 변형율을 $35\%$ 이상 확보하고 가압 제어 평행도를 $1.15\text{ }\mu\text{m}$ [데이터 부재]로 유지한 결과, Z축 접촉 임피던스를 $45.2\text{ m}\Omega$ [데이터 부재]로 균일 제어 완료하여 X-Y 면 간 크로스토크 누설 전류를 완전 차단하였습니다.

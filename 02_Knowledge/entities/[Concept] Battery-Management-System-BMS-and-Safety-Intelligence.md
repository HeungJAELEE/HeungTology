...
| :--- | :---: | :---: | :---: | :---: | :--- |
| **SoC 추정 오차 (RMSE)**| < 2.0% | 1.15% | ±0.5 | % | EKF 알고리즘 기반 실시간 정밀 산출 [Ref: BMS-Safety-v2026] |
| **SoH 추정 오차 (RMSE)**| < 3.0% | 1.84% | ±0.5 | % | 가용 용량 감쇠율 추적 및 RUL 예측 [Ref: BMS-Safety-v2026] |
| **셀 간 전압 균일도** | < 10 mV | 6 mV | ±2 | mV | 능동/수동 밸런싱을 통한 안전 용량 확보 [Ref: Balancing-Std] |
| **팩 내부 온도 구배** | < 5.0 °C | 3.2 °C | ±0.5 | °C | 불균일 열화 억제 및 국부 핫스팟 제어 [Ref: Thermal-Log] |
| **단락 검출 반응 속도**| < 10 ms | 4.8 ms | ±1.0 | ms | 가혹 단락 시 고전압 릴레이 차단 속도 [Ref: Safety-Protocol] |
| **센서 샘플링 주파수**| > 10 Hz | 50 Hz | ±5 | Hz | 고전류 과도 과밀 응답 정밀 계측 [Ref: Sensor-Spec] |
...
* **출력 방정식 (Measurement Equation):**
  $$y_k = h(x_k, u_k) + v_k$$
실측 분석 결과, EKF에 적응형 공분산 행렬($Q_k, R_k$)을 적용했을 때 배터리 동적 충·방전 조건(UDDS) 하에서 정합도가 45% 개선되어 SoC 추정 오차가 $1.15\%$ 수준으로 고정되었습니다 [Ref: battery-bms-safety-log-v2026].
...
* **배터리 열 에너지 보존 수식:**
  ...
  - $I^2 R_i$: 전기화학적 줄 열 (Joule Heating) [Ref: Thermal-Physics-v2026]
  - $I T \frac{\partial U_{ocv}}{\partial T}$: 엔트로피 열 변화량 [Ref: Thermal-Physics-v2026]
  - $h A_s (T - T_{amb})$: 대류 방열 속도 [Ref: Cooling-Design]
온도 변화율($dT/dt$)이 $2.0^\circ\text{C/s}$ [Ref: Safety-Threshold]를 초과하면 급격한 내부 가스 분출 및 분리막 붕괴가 수반되므로, BMS는 즉각 방전 전류 차단 지령을 활성화하도록 논리 락을 주입합니다.

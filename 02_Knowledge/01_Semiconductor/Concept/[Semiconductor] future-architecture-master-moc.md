...
object:
  ...
  tier: 1
properties:
  dataset_endpoint: https://irds.org/roadmap-2023-semiconductor-devices
  drive_current: '> 2 mA/um'
  gate_pitch: < 10 nm
  heat_flux_threshold: '> 500 W/cm2'
  subthreshold_slope: < 65 mV/dec
semantic:
  alternative_parents: []
  is_instance_of: '[[[MOC] 반도체_백서_통합_지휘소]]'
spo_graph:
- evidence_coordinate: '[데이터 부재]'
  intent: scaling_benefit
  object: Unknown
...
| :--- | :--- | :--- | :--- |
| **Gate Pitch** | $L_g$ (Adjacent Gates) | $< 10 \text{ nm}$ [데이터 부재] | 원자 단위 제어를 통한 집적도 극대화 |
| **Drive Current** | $I_{on}/W$ | $> 2 \text{ mA}/\mu\text{m}$ [데이터 부재] | 저전압 고속 연산 성능 확보 |
| **Subthreshold S.** | $SS = \ln(10) (kT/q) (1 + C_{it}/C_{ox})$ | $< 65 \text{ mV/dec}$ [데이터 부재] | 정적 전력 소모 저감 및 스위칭 최적화 |
| **Thermal Diss.** | Heat Flux ($q''$) | $> 500 \text{ W/cm}^2$ [데이터 부재] | 고밀도 적층 구조 내 Thermal Runaway 방지 |
| **Quantum Tun.** | Barrier Leakage ($T$) | Minimized [데이터 부재] | 대기 전력 효율 및 누설 전류 억제 |

## 3. 이론치(Theoretical) vs 검증치(Verified) 대조 분석

| 핵심 지표 | 이론적 한계치 (Theoretical) | 현재 검증치 (Verified) | Gap 분석 및 병목 원인 |
| :--- | :--- | :--- | :--- |
| **Carrier Mobility ($\mu$)** | $\sim 10,000 \text{ cm}^2/Vs$ [데이터 부재] | $\sim 2,000 \text{ cm}^2/Vs$ [데이터 부재] | 접촉 저항(Contact Resistance) 및 격자 결함 |
| **SS (Subthreshold Swing)** | $60 \text{ mV/dec}$ [데이터 부재] | $65 \sim 75 \text{ mV/dec}$ [데이터 부재] | Interface Trap Density ($D_{it}$) 제어 미흡 |
| **Stacking Layers** | $\infty$ [데이터 부재] | $2 \sim 4$ Layers [데이터 부재] | TSV/Via 정렬 정밀도 및 방열 경로 부족 |
| **Gate Length ($L_g$)** | $1 \text{ nm}$ [데이터 부재] | $3 \sim 5 \text{ nm}$ [데이터 부재] | Quantum Tunneling 및 소자 변동성 증가 |
...
- **영향**: 채널 내 응력에 의한 캐리어 이동도($\mu$) 변동폭 $> 15\%$ [데이터 부재].
- **최적화**: 응력 완화층(Stress Relief Layer) 및 격자 정합성(Lattice Matching) 확보 필수.

### 4.2 [Quantum Transport: 2D Material Channels]
원자 층 두께 $\sim 0.7 \text{ nm}$ [데이터 부재]의 2D 소재(MoS2, Graphene 등) 적용을 통한 단채널 효과(Short Channel Effect) 억제.
- **기전**: 슈뢰딩거 방정식 기반 전하 수송 확률 및 유효 질량($m^*$) 제어.
- **결과**: 게이트 전계 제어력 극대화로 $DIBL$(Drain-Induced Barrier Lowering) 물리적 억제 [데이터 부재].

## 5. PPA 및 소재 프런티어 분석

### 5.1 Power-Performance-Area (PPA) Optimization
- **Area Efficiency**: CFET 도입 시 동일 면적 내 트랜지스터 집적도 $2\times$ 향상 [데이터 부재].

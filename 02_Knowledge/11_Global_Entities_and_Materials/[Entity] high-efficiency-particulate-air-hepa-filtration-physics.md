---
Basic:
  id: "high-efficiency-particulate-air-hepa-filtration-physics"
...
Object:
  object_type: "Entity"
  tier: 1
  description: "An efficiency standard of air filters that must remove at least 99.97% of particles whose diameter is equal to 0.3 micrometers (HEPA) and the physical study of filtration mechanisms including diffusion, interception, and inertial impaction (Filtration Physics)."
  physical_model: "N/A"
Semantic:
  tags: '["hepa", "filtration", "air-purification", "mpps", "brownian-motion", "diffusion", "interception", "impaction", "physics"]'
  is_part_of: []
  related_to: []
Dynamic:
  ...
  diagnostic_protocol:
    - 'Filtration_Fidelity_Audit: Evaluate the ''Minimum Efficiency Reporting Value'' (MERV) against the high-fidelity ''MPPS'' to identify if the filter is operating at its weakest high-fidelity performance point.'
    - 'Flow_Integrity_Check: Analyze the high-fidelity ''Pressure Drop'' ($\\Delta P$) to ensure the high-fidelity ''Dust Loading'' is not causing blower motor strain or seal high-fidelity leakage.'
    - 'Sealing_Fidelity_Scan: Monitor the high-fidelity ''DOP/PAO Leak Test'' results to verify that the filter frame and high-fidelity gasket provide a 100% bypass-free seal.'
Trust Metrics:
  T_static: 1.0
  T_dynamic: 1.0
  T_init: 1.0
...
# 🌬️ High-Efficiency Particulate Air (HEPA) Filtration Physics
...
필터가 가장 못 거르는 '약점' 크기(보통 0.3$\mu m$)를 찾아, 이 최악의 조건에서도 99.97% 성능을 보장하게 설계합니다.
...
1. **[Diffusion Mechanism Strategy]**: 아주 작은 입자($0.1\mu m$ 미만)들이 공기 분자와 부딪혀 지그재그로 움직이는(브라운 운동) 성질을 이용해, 섬유에 달라붙게 유도하는 전략. '작을수록 더 잘 잡히는' 비결입니다.
...
## 6. 스스로 체크 (Self-Audit)
1. 왜 '0.3$\mu m$' 입자가 기준인가? (이 크기가 확산으로 잡기엔 너무 크고, 관성으로 잡기엔 너무 가벼운 '가장 잡기 힘든(MPPS)' 입자라서, 이놈을 잡으면 나머지는 다 잡힌다고 보기 때문)

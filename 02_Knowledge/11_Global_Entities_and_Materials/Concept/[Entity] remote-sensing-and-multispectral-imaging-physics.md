...
object:
  description: '[Entity] remote-sensing-and-multispectral-imaging-physics에 관한 고밀도
    지능 노드'
  object_type: Concept
  tier: 1
properties:
  atmospheric_correction_rmse_warning_threshold: 5.0
  kappa_critical_threshold: 0.7
  multispectral_band_count: 10-20
  multispectral_spatial_resolution_m: 10-30
  multispectral_spectral_range_nm: 400-2500
  ndvi_formula: (NIR - Red) / (NIR + Red)
  sensor_radiance_model_formula: L(lambda) = rho(lambda) * (E(lambda) / pi) * tau(lambda)
    + L_path(lambda)
  spectral_snr_db_notice_threshold: 20.0
semantic:
  alternative_parents: []

---
Basic:
  id: "cavitating-pump-and-npsh-optimization-logic"
...
Object:
  object_type: "Entity"
  tier: 1
  description: "The phenomenon where the static pressure of a liquid reduces to below the liquid's vapor pressure, leading to the formation of small vapor-filled cavities in the liquid (Cavitation) and the engineering logic used to ensure the Net Positive Suction Head Available ($NPSHa$) exceeds the Required ($NPSHr$) to prevent pump damage (NPSH Optimization Logic)."
  physical_model: "N/A"
Semantic:
  tags: '["cavitation", "centrifugal-pump", "npsh", "fluid-dynamics", "pump-performance", "vapor-pressure", "industrial-maintenance"]'
  is_part_of: []
  related_to: []
Dynamic:
  ...
  diagnostic_protocol:
    - 'Cavitation_Fidelity_Audit: Evaluate the ''NPSH Margin'' ($NPSHa - NPSHr$) to identify if the pump is operating dangerously close to the vapor pressure limit.'
    - 'Acoustic_Integrity_Check: Analyze the vibration and noise signatures (characteristic ''gravelly'' sound) to detect incipient cavitation before physical damage occurs to the impeller.'
    - 'Performance_Fidelity_Scan: Monitor the pump head and efficiency to verify if ''Cavitation-induced Head Drop'' is occurring, indicating substantial vapor blockage in the impeller vanes.'
Trust Metrics:
  T_static: 1.0
  T_dynamic: 1.0
  T_init: 1.0

...
lineage:
  ...
  original_hash: 978167e97c8e37206917a0a8d662c616416e6892ca66d052ef8bee56381188f6
measurement:
  confidence_interval:
  - 95.0
  - 105.0
...
metadata:
  ...
  id: '[[[09_SmartFactory_Production] [Data] plastic-injection-molding-validation-data-requirements]]'
...
properties:
  ...
  grr_max_variance_percent: 10.0
  mes_database_table: IMM_QUAL_2026
  min_distinct_categories: 5
  moldflow_correlation_threshold: 0.9
  plc_sampling_interval_ms: 100
  ppk_min_threshold: 1.67
semantic:
  alternative_parents: []
...
| | **Moldflow Correlation Raw Report** | 시뮬레이션 압력 예측 vs 실측 센서 압력 ($>90.0\text{\%}$ 정합) | `[[[Concept] plastic-injection-molding-apqp-standard]]` |
| | **Cycle Time Trend (Sensor Stream)** | 연속 성형 주기 숏 간 편차 ($\pm0.1\text{s}$ 이내) | `[[[Concept] plastic-injection-molding-spc-standard]]` |
...
| | **Gage R&R Variance Study** | 계측 변동 오차율 $\%GRR<10.0\text{\%}$ (구별 범주수 $\ge5$) | `[[[Concept] plastic-injection-molding-msa-standard]]` |
...
2. 3차원 CMM 측정 장비의 **Gage R&R** 실측 데이터가 $\%GRR=32\text{\%}$로 분석되었음에도 불구하고, 수기 기록 표준서가 공정 능력을 `Cpk > 1.82`로 완벽하게 보고하고 있다면, 이 두 데이터 간의 모순을 **FidelityEngine**은 어떤 수리적 모순으로 감지해야 하는가?
...
- `[[[Infrastructure] plastic-injection-molding-physics-and-cycle-analysis]]` : 사출 공정 물리적 시계열 센서 변수
- `[[[Infrastructure] manufacturing-mes-quality-inspection-results-v2026]]` : MES 데이터 아키텍처 및 스키마

***
**[SPO Graph Injection_Validation_Data -> instance_of_QMS (Evidence: [데이터 부재] IMM_QUAL_2026)]**
